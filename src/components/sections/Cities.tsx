  ...
  return (
    <section className="py-12 sm:py-16 md:py-24 bg-[#6b9834]/5 relative">
      ...
      <Container>
        ...
        <StaggerContainer className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 sm:gap-3 md:gap-4" staggerDelay={0.05}>
          {displayedCities.map((city) => (
            <StaggerItem key={city.slug}>
              ...
              >
                <Link
                  href={`/${city.slug}/`}
                  className="group flex items-center gap-1.5 sm:gap-2 p-3 sm:p-4 bg-white rounded-lg sm:rounded-xl hover:bg-[#6b9834] transition-all duration-300 shadow-sm hover:shadow-md min-h-[44px]"
                >
